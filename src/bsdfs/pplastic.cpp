...
public:
    ...
    Spectrum eval(const BSDFContext &ctx, const SurfaceInteraction3f &si,
        ...
        if constexpr (is_polarized_v<Spectrum>) {
            ...
            if (has_diffuse) {
                ...
                // Refract inside
                Spectrum To = mueller::specular_transmission(abs(Frame3f::cos_theta(wo_hat)), m_eta);
                ...
                result += diff * math::InvPi<Float> * cos_theta_o;
            }
        } else {
            if (has_specular) {
                MicrofacetDistribution distr(m_type, m_alpha_u, m_alpha_v, m_sample_visible);
    ...
    Float pdf(const BSDFContext &ctx, const SurfaceInteraction3f &si,
        ...
        if (m_sample_visible)
            ...
            p_specular = distr.pdf(si.wi, H) / (4.f * dot(wo, H));
        masked(p_specular, dot(si.wi, H) <= 0.f || dot(wo, H) <= 0.f) = 0.f;

        // Diffuse component
        Float p_diffuse = warp::square_to_cosine_hemisphere_pdf(wo);

        return select(active, prob_specular * p_specular + prob_diffuse * p_diffuse, 0.f);
    }
